...
/**
 * arieles-block block decorator. -- THIS IS A KEY BLOCK THAT DETERMINES THE BUSINESS LOGIC, among other things. 
...
export default function decorate(block) {
  ...
  const layoutStyle = readProp('arieles-blocklayout', 3) || 'overlay';
  ...
  // arieles-block uses explicit CTA fields (CTA Button Label + Link). Any hyperlink the
  ...
  if (textDiv) textDiv.classList.add('arieles-block-text');

  // --- Auto-detect video from any <a> in the block ---
  const videoLink = findVideoLink(block);

  if (videoLink) {
    block.classList.add('arieles-block-video');
    ...
    video.className = 'arieles-block-video-bg';
  ...
  if (badgeText && textDiv) {
    ...
    badgeEl.className = 'arieles-block__badge';
  ...
  //     badge (a lone badge should still render above an otherwise-empty arieles-block) ---
