...
function isVideoLink(link) {
  ...
  try {
    ...
    if (VIDEO_EXTENSIONS.some((ext) => assetName.toLowerCase().endsWith(ext)))
      return true;
  } catch (_) {
...
export default function decorate(block) {
  ...
  const readProp = (prop, index) => {
    const authored = block.querySelector(
      `:scope > div [data-aue-prop="${prop}"]`,
    );
  ...
  const layoutStyle = readProp("speakerlayout", 3) || "overlay";
  ...
  const ctaLinkDiv =
    childDivs.slice(2).find((d) => d.querySelector("a")) || childDivs[6];
  const ctaLinkAnchor = ctaLinkDiv?.querySelector("a");
  const ctaLink =
    ctaLinkAnchor?.getAttribute("href") ||
  ...
  // speaker uses explicit CTA fields (CTA Button Label + Link). Any hyperlink the
  ...
  if (textDiv) textDiv.classList.add("speaker-text");

  // --- Auto-detect video from any <a> in the block ---
  const videoLink = findVideoLink(block);

  if (videoLink) {
    block.classList.add("speaker-video");
    ...
    video.className = "speaker-video-bg";
  ...
  if (badgeText && textDiv) {
    ...
    badgeEl.className = "speaker__badge";
  ...
  if (
    assetDiv &&
    assetDiv.textContent.trim() === "" &&
    !assetDiv.querySelector("picture, video")
  ) {
    assetDiv.style.display = "none";
  }

  // --- Hide the text overlay div only if it has no meaningful content AND no
  //     badge (a lone badge should still render above an otherwise-empty speaker) ---
